...
Public Class MasterOwner
    ...
    Private Sub MasterOwner_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.CenterToScreen()
        clear()
        koneksi()
        tampil_id()
    ...
    Private Sub MasterOwner_Activated(sender As Object, e As EventArgs) Handles MyBase.Activated
        koneksi()
        data_owner()
        tampil_id()
    End Sub
    ...
    Sub data_owner()
        ...
        Try
            ds.Clear()
            Dim query As String = "SELECT id_user, nama, password, email FROM tbl_user WHERE id_user LIKE '%OWN%'"
    ...
    Sub tampil_id()
        ...
        da = New MySqlDataAdapter("Select Case When MAX(CONVERT(SUBSTRING(id_user, 4), SIGNED)) Is NULL Then 
                                    'OWN001' ELSE CONCAT('OWN', LPAD(MAX(CONVERT(SUBSTRING(id_user, 4), SIGNED)) + 1, 3, '0')) 
                                    END AS next_id_user FROM tbl_user WHERE id_user LIKE 'OWN%';", conn)
        da.Fill(ds, "id")
        id = ds.Tables("id").Rows(0).Item(0).ToString
        TbIDAdmin.Text = id
    End Sub

    Private Sub BtnTambahOwner_Click(sender As Object, e As EventArgs) Handles BtnTambahOwner.Click
    ...
    Private Sub BtnUbahOwner_Click(sender As Object, e As EventArgs) Handles BtnUbahOwner.Click
